 ...
 *
 * Five 16:9 backgrounds (Preset A to Preset E), each with its own
 ...
 *   • night   (Preset C) -> dark obsidian night, glowing neon amber accents,
...
function pfpCircle(ctx, av, cx, cy, r, ringColor, glowColor){
  ctx.save();
  if(glowColor){
    ctx.shadowColor = glowColor;
    ctx.shadowBlur = 45;
...
export async function renderScorecard(variant, d){
  ...
  const isNight = variant.id === 'night';
  const [bg, lg, lgW, av] = await Promise.all([
    loadImage(variant.bg),
    loadImage(logo),
    isNight ? loadImage(logoWhite) : Promise.resolve(null),
